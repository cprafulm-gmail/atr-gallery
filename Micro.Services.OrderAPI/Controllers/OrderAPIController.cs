    ...
    public class OrderAPIController : ControllerBase
    {
        ...
        public ResponseDto? Get(string? userId = "")
        {
            try
            {
                ...
                {
                    objList = _db.OrderHeaders.Include(u => u.OrderDetails).OrderByDescending(u => u.OrderHeaderId).AsNoTracking().ToList();
                }
                else
                {
                    objList = _db.OrderHeaders.Include(u => u.OrderDetails).Where(u=>u.UserId == userId).OrderByDescending(u => u.OrderHeaderId).AsNoTracking().ToList();
                }
        ...
        //Updating the status of an order, If new status is set to
        //"Cancelled" initiates a refund for the corresponding payment using Stripe 
        [Authorize]
